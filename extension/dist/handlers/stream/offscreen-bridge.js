...
function assertCapturableUrl(url) {
    if (!url)
        return;
    const blocked = url.startsWith('chrome://') ||
        url.startsWith('chrome-extension://') ||
        url.startsWith('edge://') ||
        url.startsWith('about:') ||
        url.startsWith('devtools://');
    if (blocked) {
        throw new Error(`Cannot capture tab URL "${url}": privileged pages (chrome://, about:, extensions) are not capturable with tabCapture.`);
    }
}
export async function captureTabInOffscreen(tabId) {
...
export async function teardownOffscreenCapture() {
    try {
