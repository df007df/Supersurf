...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    ...
    if (message.type === 'livePreviewMouse') {
        ...
        if (compositor && message.tabId === activeTabId) {
            ...
            applyCompositorMouse(compositor, kind, Number(message.x) || 0, Number(message.y) || 0, Number.isFinite(cssWidth) && cssWidth > 0 ? cssWidth : undefined, Number.isFinite(cssHeight) && cssHeight > 0 ? cssHeight : undefined);
        }
        return false;
    }
